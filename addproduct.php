...
<body>
    ...
    <p align="center">Please Fill in this Form to Setll a product.</p>
    <hr>
   ...
   <?php
            if(isset($_POST['submit']))
            {
                if(isset($_SESSION['id']))
                {
                     ...
                     try{
                      ...
                      $query =$dbhandler->query("SELECT MAX(PROD_ID) FROM product");
                      $sel = $query->fetch();
                      
                      echo $sel['MAX(PROD_ID)'];
                       $proid = $sel['MAX(PROD_ID)'] + 1;
    
                       $sql = "INSERT INTO images(image_id,image,PROD_ID) values(' ','$image','$proid')";
     ...
     // Insert record
                     $sql="INSERT INTO product(PROD_ID, SELLER_ID, SUBCAT_ID, PROD_NAME, DISCOUNT, PRICE, PICTURE) 
                                VALUES ('', '$uid' , '$sel', '$prod_name', '$discount', '$price', '$image')";
            $query=$dbhandler->query($sql);
